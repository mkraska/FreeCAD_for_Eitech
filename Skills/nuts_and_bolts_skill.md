...
- ✅ Makro: Schrauben/Muttern/Gewindestifte (`nuts_and_bolts.py` v1.1 – robuste Sub-Assembly-Auflösung, korrekte Joint-Referenzen, Mutter-Verdrehung über Offset2)
...
- **Fixed Joint per Python**:
  ...
  - **SubElementName muss RELATIV zum Referenz-Objekt sein** (nicht dessen eigenen Namen als Präfix enthalten!). `Gui.Selection.getSelectionEx('',0)`-`SubElementNames` sind relativ zum GANZ OBEN selektierten Objekt, nicht relativ zu einem Zwischen-Link. Wird ein Zwischen-Link als `ReferenceX`-Objekt verwendet, aber der volle (unveränderte) `SubElementName` übergeben, schlägt `obj.getSubObject(subname)` mit `None` fehl → Solver berechnet `PlacementX` nicht aus der Geometrie → **falsche/zufällige Placement, OHNE Fehlermeldung im Log**. Diagnose: `ref_obj.getSubObject(voller_name)` → `None` = Bug.
  - **`Placement1`/`Placement2` werden bei JEDEM `recompute()` aus `Reference1`/`Reference2` neu vom Solver berechnet** – eine nur direkt in `obj.Placement` gesetzte Rotation wird dabei kommentarlos überschrieben. **`Offset1`/`Offset2` werden dagegen vom Solver respektiert und bleiben über `recompute()` hinweg erhalten** → einziger zuverlässiger Ort für dauerhafte Rotations-/Positionsanpassungen (z.B. Verdrehung eines Teils um seine eigene Achse). An der Konsole verifiziert: `Offset2`-Rotationswinkel-Delta zwischen zwei Werten entspricht exakt dem gesetzten Delta, `Base` bleibt stabil.
  - **`getSubObjectList(subname)`** liefert die Objekt-KETTE (keine Shape-Transformation) vom aufrufenden Objekt bis zum Blatt-Objekt, das das referenzierte Element (Edge/Face) tatsächlich besitzt – zuverlässiger als manuelles Pfad-Parsen über mehrere Sub-Assembly-Ebenen. Der Edge-Index aus dem letzten `SubElementName`-Segment (`'Edge150'` → Index 149) ist direkt im **lokalen** `Shape.Edges` dieses Blatt-Objekts gültig, unabhängig von der Verschachtelungstiefe.
- **`ancestorsOfType` kann bei tief verschachtelten/gespiegelten Sub-Assemblies fehlschlagen**: `NCollection_IndexedDataMap::FindFromKey` bzw. `<TopoShape> hasher mismatch`, weil die selektierte Edge eine transformierte Kopie ist, die nicht exakt mit der Shape-Identität übereinstimmt, gegen die gesucht wird. Fallback: geometrisches Matching (Center/Achse/Radius statt Shape-Identität) über `shape.Faces` / `face.Edges`.
...
### Schraube-Einfügen: Vorzeichen der Achse (Materialseite) bestimmen
Ziel: herausfinden, welche Seite der selektierten Kreiskante zum Material zeigt
(Kopfauflage vs. freies Ende). Kriterium ist ein reiner Vorzeichenvergleich
(Dot-Product `cog_vec · axis`) → **transform-invariant**, braucht KEINE
Weltkoordinaten, nur ein für Kante+Fläche konsistentes KS.

**Bester Weg** (schnell, robust, kein Weltkoord-Transform nötig):
1. `raw_obj.getSubObjectList(edge_name)` → Objekt-Kette, letztes Element =
   tatsächliches Blatt-Bauteil (z.B. `Kühlergrill_LKW`)
2. Edge-Index aus letztem SubElementName-Segment (`'Edge150'` → `Shape.Edges[149]`)
   **direkt im lokalen Shape des Blatt-Objekts**
3. `local_shape.ancestorsOfType(local_edge, Part.Face)` darauf ausführen →
   klein, schnell, kein `hasher mismatch`
4. Radius-Sanity-Check (`local_edge.Curve.Radius` vs. Original-Kante) als
   Plausibilitätsprüfung

Fallback-Kaskade (schnellster zuerst): Blatt-Objekt (lokal, s.o.) →
`target_link` (eine Ebene über `raw_obj.getSubObject(objekt_pfad)`, ebenfalls
welt-transformiert, aber unzuverlässig bei den kryptischen Hash-Element-Namen
`;#42c5:2;:Hbc5,E;...` – liefert dann oft `None`) → `raw_obj` (ganzes Modell,
letzter Fallback, langsam + viele `hasher mismatch`-Warnungen).

Ein Bauteil DIREKT ansprechen (z.B. `Kühlergrill_LKW.Shape`, ohne über
`getSubObjectList`/`getSubObject` zu gehen) liefert nur dessen EIGENES lokales
KS, OHNE die Placements der Elternebenen – für Weltkoordinaten ungeeignet, nur
für die reine (transform-invariante) Vorzeichen-Bestimmung nutzbar.

### Mutter-Einfügen: Verdrehung
- Zufallsverdrehung bei Sechskantmutter NICHT auf Vielfache von 60° würfeln
  (6-zählige Symmetrie → alle sehen identisch aus!), sondern
  `random.uniform(1e-6, 60.0)` – Wert echt INNERHALB einer 60°-Periode
- Verdrehung gehört in `joint.Offset2` (Rotation), nicht in `mutter_link.Placement`
  direkt (siehe "Fixed Joint per Python" oben)

### Bugfix-Historie (Juli 2026)
- **Crash** (`not enough values to unpack`): alte `_resolve_faces()`-Hilfsfunktion
  (manuelle Pfad-Auflösung über mehrere Sub-Assembly-Dokumente) hatte
  `return`-Statements mit inkonsistenter Tuple-Länge. Komplett ersetzt durch
  die `getSubObjectList()`-Methode oben.
- **Falsche Schrauben-Position** bei tief verschachtelten Sub-Assemblies:
  `Reference2` zeigte auf einen Zwischen-Link, aber der übergebene
  `SubElementName` enthielt dessen eigenen Namen noch als Präfix →
  `getSubObject()` → `None` → Solver konnte `Placement2` nicht berechnen.
  Fix: `ReferenceX` direkt auf das per `getSubObjectList()` aufgelöste
  Blatt-Objekt setzen (simpler lokaler Element-Name, kein Präfix-Problem).
- **Muttern-Verdrehung geht verloren** (Kontermutter-Paar ohne relative
  Verdrehung): Rotation wurde nur in `mutter_link.Placement` gesetzt (vom
  Solver bei `recompute()` überschrieben) statt in `joint.Offset2` (bleibt
  erhalten). Siehe "Fixed Joint per Python" oben.
