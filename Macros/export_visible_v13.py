...
Änderungen gegenüber v12 (export_visible.py):
- BUGFIX Farbkopie: collect_source_colors() hat bisher nur DiffuseColor
  ...
  FreeCAD-Standardwerte ersetzt. Sichtbar geworden bei Zylindersegment032-
  035 in Gleiter.FCStd (dreistufige App::Link-Kette auf ../Teile/
  Plastik.FCStd, Body032) - Farbe im Export/Viewer wich vom Original ab.
  Jetzt wird das komplette Material (alle Kanäle) übernommen. Wichtig:
  das betrifft NUR die für den Export neu angelegte, unabhängige
  Part::Feature-Kopie - die von paint.py dokumentierte FreeCAD-
  Einschränkung (App::Link-Instanzen rendern bei OverrideMaterial=True
  live nur Diffuse pro Instanz, siehe FreeCAD/FreeCAD#19135) gilt dort
  nicht, weil es sich um ein eigenständiges Objekt ohne Link-Override-
  Mechanik handelt - das gespeicherte Material kann also 1:1 übernommen
  werden.
...
Nur als export_visible_v13.py angelegt (nicht export_visible.py
überschrieben), damit der bisherige, bekannt funktionierende Stand
(v12) für Vergleiche/Rückfall erhalten bleibt.
